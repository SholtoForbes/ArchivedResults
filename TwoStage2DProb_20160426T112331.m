...
% These should be zero if the state or control is free within set bounds
% (other conditions sometimes apply)
mu_1 = dual.states(1,:);
mu_2 = dual.states(2,:);
mu_3 = dual.states(3,:);
mu_4 = dual.states(4,:);
...
theta_F = integral(thetadot,min(t),max(t));

v_F = integrate(a.*sin(theta_F),min(t),max(t));

V_F = integrate(v_F,min(t),max(t));

mfuel_F = integrate(Fueldt,min(t),max(t));
