...
ThrustF_spline= scatteredInterpolant(enginedata(:,1),enginedata(:,2),enginedata(:,3),'natural'); %interpolator for engine data (also able to extrapolate badly)
global FuelF_spline
FuelF_spline= scatteredInterpolant(enginedata(:,1),enginedata(:,2),enginedata(:,4),'natural'); %interpolator for engine data
...
flapmoment_interp = scatteredInterpolant(communicator_trim(:,1),communicator_trim(:,2),communicator_trim(:,3),communicator_trim(:,4),'natural');
global flap_interp
flap_interp = scatteredInterpolant(communicator_trim(:,1),communicator_trim(:,2),communicator_trim(:,4),communicator_trim(:,3),'natural');
global flapdrag_interp
flapdrag_interp = scatteredInterpolant(communicator_trim(:,1),communicator_trim(:,2),communicator_trim(:,3),communicator_trim(:,5),'natural');
AoA_momentinterp = scatteredInterpolant(communicator(:,1),communicator(:,2),communicator(:,11),'natural');



ThirdStageData = dlmread('thirdstage.dat');
% global ThirdStagePayloadSpline
ThirdStagePayloadSpline = scatteredInterpolant(ThirdStageData(:,1),ThirdStageData(:,2),ThirdStageData(:,3),ThirdStageData(:,5),'natural'); % not actually a spline...
...
omegadot = diff(transpose(smooth([0,diff(transpose(smooth(theta)))./(diff(t))])))./diff(t);
